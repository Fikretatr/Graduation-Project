...
State* Fsd::addState(QPointF pos, QString id,QString description)
{
...
void Fsd::mousePressEvent(QGraphicsSceneMouseEvent *mouseEvent)
{
    ...
    switch ( mode ) {
        ...
        case SelectItem:
          ...
          break;
        case ConvertJtoVer:
            cout<< FileNameJson.toStdString() << endl;
            convert_jtover();
            break;
    }
}
...
void Fsd::fromString(QString& json_text)
{
    ...
    clear(); 
    initData = "";
    if(json.find("init") != json.end()){
        initData = json.at("init");
    }
    ...
    for ( const auto json_state : json.at("states") ) {
      ...
      std::string description = json_state.at("description");
      State* state;
      if ( id == State::initPseudoId.toStdString() )
        state = addPseudoState(QPointF(json_state.at("x"), json_state.at("y")));
      else 
        state = addState(QPointF(json_state.at("x"), json_state.at("y")), QString::fromStdString(id), QString::fromStdString(description));
...
QString Fsd::toString()
{
    ...
    for ( const auto item: items() ) {
      if ( item->type() == State::Type ) {
        ...
        json["description"] = state->getDes().toStdString();
...
void Fsd::convert_jtover(){
   // QString Filename=MainWindow.get_filename();
   converter.readFile(FileNameJson,isFromCCode);
   QString vLogFileName = "C:\\Users\\atarf\\Desktop\\ssde-master\\olusturulan\\outputVerilog.txt";
   converter.toVlog(vLogFileName,isFromCCode);
