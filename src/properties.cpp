...
PropertiesPanel::PropertiesPanel(MainWindow* parent) : QFrame(parent)
{
    ...
    connect(Deslabel_field, &QLineEdit::textEdited, this, &PropertiesPanel::setStateDescription);
...
void PropertiesPanel::createStatePanel()
{
    ...
    Deslabel_field = new QLineEdit();
    statePanelLayout->addWidget(Deslabel);
    statePanelLayout->addWidget(Deslabel_field);
...
void PropertiesPanel::setSelectedItem(State* state)
{
    ...
    if ( ! state->isPseudo() ) {
      ...
      Deslabel_field->setText(state->getDes());
      }
}
...
void PropertiesPanel::setStateDescription(const QString& Description)
{
    State* state = qgraphicsitem_cast<State*>(selected_item);
    if(state != nullptr) {
